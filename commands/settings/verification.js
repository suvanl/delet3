...
import { verifSettingsKey, friendlySettings } from "../../core/util/data";

export const run = async (client, message) => {
    const cat = "verification";
    const msg = stripIndents`
        ⚙️ ${client.l10n(message, "settings.prompt.cat").replace(/%category%/g, cat)}

        1️⃣ **${client.l10n(message, "settings.verificationChannel")}**
        2️⃣ **${client.l10n(message, "settings.verificationEnabled")}**
    ...
    if (!num.includes(selected)) return message.channel.send(client.l10n(message, "settings.invalidNum").replace(/%range%/g, "1-2"));
        ...
        const setting = verifSettingsKey[selected];

        // Relate number to message.settings
        const s = {
            1: message.settings.verificationChannel,
            2: message.settings.verificationEnabled
        };
        ...
        try {
            ...
            return client.logger.error(`Error changing a verification system setting:\n${err.stack}`);   
        }
    }
};
...
export const help = {
    name: "verification",
    description: "changes delet's verification system settings on the current server",
    category: "settings",
    usage: "verification"
};
